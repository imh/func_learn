...
while True:
    ...
    n_ok = 0
    tries = 0
    while not (n_ok >= 100 and tries >= 100):
        context['depth'] = 0
    ...
    if len(ok_errs) > 0:
        ...
        # print 'added losses'
        print 'LOSS_OK (min={:.1f}, mean={:.1f}, max={:.1f}),'.format(min_ok_errs, mean_ok_errs, max_ok_errs), '{} / {} ok.'.format(len([lc for lc in lcs if lc.ok]), len(lcs)),
        loss.backward()
